...
// v5.8.2 vertical precision aiming. Coarse direction still comes only from a
// completed table tap/click. The dedicated right-side roller is a relative
// vertical control inspired by pool-game aim wheels: drag UP/DOWN to rotate the
// cue continuously. Merely touching the roller never changes the shot angle.
...
function bindVerticalAimWheel(el,setVisual){
  if(!el)return;
  let active=false,pid=null,startX=0,startY=0,lastY=0,moved=false,visual=0;
  const reset=()=>{active=false;pid=null;moved=false;visual=0;setVisual?.(0);};
  el.addEventListener('pointerdown',e=>{
    if(!canHumanAim())return;
    e.preventDefault();audio.unlock();active=true;pid=e.pointerId;startX=e.clientX;startY=lastY=e.clientY;moved=false;visual=0;setVisual?.(0);
    el.classList.add('dragging');el.setPointerCapture?.(e.pointerId);
  });
  el.addEventListener('pointermove',e=>{
    if(!active||e.pointerId!==pid||!canHumanAim())return;
    e.preventDefault();
    const coalesced=e.getCoalescedEvents?.();const samples=coalesced?.length?coalesced:[e];
    let totalDy=0;
    for(const sample of samples){const dy=sample.clientY-lastY;lastY=sample.clientY;if(Number.isFinite(dy))totalDy+=dy;}
    const travel=Math.hypot(e.clientX-startX,e.clientY-startY);
    if(!moved&&travel<1.5)return;
    moved=true;
    if(Math.abs(totalDy)<.01)return;
    // Slow movement stays highly precise. Faster swipes gain only moderately,
    // allowing larger corrections without making thin cuts twitchy.
    const speed=Math.min(12,Math.abs(totalDy));
    const gain=e.shiftKey?.00012:(.00030+speed*.000010);
    cue.angle-=totalDy*gain;
    visual=(visual+totalDy)%18;
    setVisual?.(visual);
    sendOnlineAim(onlineAimPullback);
    view.render?.();
  });
  const end=e=>{
    if(!active||(e&&e.pointerId!==pid))return;
    e?.preventDefault?.();
    // No tap-nudge here: touch + release without a drag is intentionally inert.
    // This prevents the same accidental-aim problem the table input was fixed for.
    if(moved&&canHumanAim())sendOnlineAim(onlineAimPullback,true);
    try{if(e?.pointerId!=null&&el.hasPointerCapture?.(e.pointerId))el.releasePointerCapture(e.pointerId);}catch(_){}
    el.classList.remove('dragging');reset();
  };
  el.addEventListener('pointerup',end);
  el.addEventListener('pointercancel',end);
  el.addEventListener('lostpointercapture',()=>{if(active){el.classList.remove('dragging');reset();}});
}
bindVerticalAimWheel(aimDisc,v=>{aimDisc?.style.setProperty('--aim-roll',`${v}px`);if(aimNeedle)aimNeedle.style.transform='translate(-50%,-50%)';});
bindVerticalAimWheel(mobileAimWheel,v=>{mobileAimWheel?.style.setProperty('--aim-roll',`${v}px`);if(mobileAimRotor)mobileAimRotor.style.backgroundPosition=`center ${v}px`;});
...
async function launchHubMode(mode){audio.unlock();if(onlineMode){online.leave();onlineMode=false;onlineReady=false;onlineSeat=null;onlineRoomCode='';$('#matchMode').value='ai';aiMode=true;}startMode(mode);if($('#autoFullscreen').checked&&!document.fullscreenElement){try{await document.documentElement.requestFullscreen?.();}catch(_){showStatus('PRESS F11 FOR FULLSCREEN',1300);}}}
window.__cueLaunchMode=launchHubMode;
for(const card of document.querySelectorAll('.r5-mode')){card.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();launchHubMode(card.dataset.mode);});card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();launchHubMode(card.dataset.mode);}});}
...
class Renderer2D{
  ...
  #background(c,w,h){
    // Tournament hall: deep graphite/green-black with restrained warm venue light.
    const base=c.createLinearGradient(0,0,0,h);base.addColorStop(0,'#141812');base.addColorStop(.24,'#0a0e0b');base.addColorStop(.70,'#040604');base.addColorStop(1,'#010201');c.fillStyle=base;c.fillRect(0,0,w,h);
    const arena=c.createRadialGradient(w*.5,h*.16,8,w*.5,h*.31,Math.max(w,h)*.74);arena.addColorStop(0,'rgba(207,181,120,.17)');arena.addColorStop(.27,'rgba(45,121,88,.10)');arena.addColorStop(1,'rgba(0,0,0,0)');c.fillStyle=arena;c.fillRect(0,0,w,h);
    // Low-contrast tournament carpet keeps the scene dense without visual noise.
    const carpet=this.#pattern('arena-carpet',52,(x,n)=>{x.fillStyle='#080b08';x.fillRect(0,0,n,n);x.strokeStyle='rgba(172,153,111,.045)';x.lineWidth=1;for(let i=-n;i<n*2;i+=13){x.beginPath();x.moveTo(i,0);x.lineTo(i+n,n);x.stroke();x.beginPath();x.moveTo(i,n);x.lineTo(i+n,0);x.stroke();}x.fillStyle='rgba(255,255,255,.018)';for(let y=5;y<n;y+=10)for(let z=5;z<n;z+=10)x.fillRect(z,y,1,1);});
    c.save();c.globalAlpha=.48;c.fillStyle=carpet;c.fillRect(0,h*.40,w,h*.60);c.restore();
    // Warm-white overhead panels: professional televised table, not neon arcade light.
    const pw=Math.min(250,w*.215),ph=Math.max(6,h*.011);for(const dx of [-.24,0,.24]){const x=w*.5+dx*w-pw*.5,y=Math.max(8,h*.072);c.save();c.shadowColor='rgba(224,197,137,.26)';c.shadowBlur=24;c.fillStyle='rgba(242,232,204,.26)';rr(c,x,y,pw,ph,ph*.5);c.fill();c.restore();}
    const floorGlow=c.createRadialGradient(w*.5,h*.70,0,w*.5,h*.70,Math.max(w,h)*.42);floorGlow.addColorStop(0,'rgba(43,116,84,.055)');floorGlow.addColorStop(1,'rgba(0,0,0,0)');c.fillStyle=floorGlow;c.fillRect(0,0,w,h);
    const vign=c.createRadialGradient(w*.5,h*.45,Math.min(w,h)*.18,w*.5,h*.48,Math.max(w,h)*.72);vign.addColorStop(.52,'rgba(0,0,0,0)');vign.addColorStop(1,'rgba(0,0,0,.72)');c.fillStyle=vign;c.fillRect(0,0,w,h);
  }
  #table(c,L){
    ...
    c.shadowColor='rgba(0,0,0,.94)';c.shadowBlur=46;c.shadowOffsetY=18;c.fillStyle='#010201';rr(c,outer.x-6,outer.y-4,outer.w+12,outer.h+14,outer.r+5);c.fill();c.shadowColor='transparent';
    const under=c.createRadialGradient(L.cx,L.cy+outer.h*.36,10,L.cx,L.cy+outer.h*.28,outer.w*.50);under.addColorStop(0,pool?'rgba(48,117,91,.095)':'rgba(42,126,88,.10)');under.addColorStop(1,'rgba(0,0,0,0)');c.fillStyle=under;c.fillRect(outer.x-40,outer.y-20,outer.w+80,outer.h+70);

    // Deep cabinet/apron layers.
    const apron=c.createLinearGradient(0,outer.y,0,outer.y+outer.h);apron.addColorStop(0,'#201d17');apron.addColorStop(.18,'#15130f');apron.addColorStop(.72,'#090a08');apron.addColorStop(1,'#030403');c.fillStyle=apron;rr(c,outer.x,outer.y,outer.w,outer.h,outer.r);c.fill();
    c.fillStyle=pool?'rgba(95,86,61,.30)':'rgba(105,85,54,.30)';rr(c,outer.x+5,outer.y+6,outer.w-10,outer.h-14,Math.max(8,outer.r-4));c.fill();
    const railSkin=c.createLinearGradient(outer.x,outer.y,outer.x+outer.w,outer.y+outer.h);railSkin.addColorStop(0,'#3a3326');railSkin.addColorStop(.18,'#211d16');railSkin.addColorStop(.5,'#11120f');railSkin.addColorStop(.82,'#29231a');railSkin.addColorStop(1,'#090a08');c.fillStyle=railSkin;rr(c,outer.x+rail*.12,outer.y+rail*.10,outer.w-rail*.24,outer.h-rail*.20,Math.max(8,outer.r-rail*.08));c.fill();

    // Procedural cabinet grain / brushed material.
    const grain=this.#pattern(pool?'rail-brushed':'rail-wood',64,(x,n)=>{x.clearRect(0,0,n,n);for(let y=2;y<n;y+=4){x.strokeStyle=y%8?'rgba(229,211,167,.030)':'rgba(0,0,0,.14)';x.beginPath();x.moveTo(0,y+Math.sin(y*.5)*1.2);x.bezierCurveTo(n*.25,y-2,n*.7,y+2,n,y);x.stroke();}});c.save();rr(c,outer.x+rail*.12,outer.y+rail*.10,outer.w-rail*.24,outer.h-rail*.20,Math.max(8,outer.r-rail*.08));c.clip();c.globalAlpha=.92;c.fillStyle=grain;c.fillRect(outer.x,outer.y,outer.w,outer.h);c.restore();

    // Layered metallic/chrome and gold inlays.
    c.strokeStyle='rgba(194,157,88,.74)';c.lineWidth=Math.max(2,rail*.080);rr(c,outer.x+2,outer.y+2,outer.w-4,outer.h-4,outer.r-2);c.stroke();
    c.strokeStyle='rgba(235,219,178,.52)';c.lineWidth=Math.max(1,rail*.036);rr(c,cloth.x-rail*.56,cloth.y-rail*.56,cloth.w+rail*1.12,cloth.h+rail*1.12,rail*.28);c.stroke();
    c.strokeStyle='rgba(119,91,47,.34)';c.lineWidth=Math.max(3,rail*.11);rr(c,cloth.x-rail*.67,cloth.y-rail*.67,cloth.w+rail*1.34,cloth.h+rail*1.34,rail*.34);c.stroke();

    // Cloth base + directional nap + weave texture.
    const felt=c.createLinearGradient(cloth.x,cloth.y,cloth.x+cloth.w,cloth.y+cloth.h);felt.addColorStop(0,pool?'#1b776b':'#16845a');felt.addColorStop(.35,pool?'#105f58':'#0e704d');felt.addColorStop(.72,pool?'#0a4b47':'#095a3f');felt.addColorStop(1,pool?'#063936':'#06442f');c.fillStyle=felt;c.fillRect(cloth.x,cloth.y,cloth.w,cloth.h);
    const weave=this.#pattern(pool?'felt-blue':'felt-green',18,(x,n)=>{x.clearRect(0,0,n,n);x.strokeStyle='rgba(255,255,255,.045)';x.lineWidth=.7;for(let i=1;i<n;i+=3){x.beginPath();x.moveTo(0,i+.5);x.lineTo(n,i+.5);x.stroke();}x.strokeStyle='rgba(0,12,8,.060)';for(let i=0;i<n;i+=4){x.beginPath();x.moveTo(i,0);x.lineTo(i,n);x.stroke();}});c.save();c.globalAlpha=.68;c.fillStyle=weave;c.fillRect(cloth.x,cloth.y,cloth.w,cloth.h);c.restore();
    ...
    const light=c.createRadialGradient(L.cx-cloth.w*.12,L.cy-cloth.h*.20,0,L.cx,L.cy,cloth.w*.58);light.addColorStop(0,'rgba(247,238,213,.13)');light.addColorStop(.48,'rgba(255,255,255,.025)');light.addColorStop(1,'rgba(0,12,8,.20)');c.fillStyle=light;c.fillRect(cloth.x,cloth.y,cloth.w,cloth.h);
    ...
    // Table markings.
    c.strokeStyle='rgba(239,238,224,.43)';c.lineWidth=1;
    ...
    for(let i=1;i<=7;i++){const x=cloth.x+cloth.w*i/8;for(const y of [cloth.y-rail*.49,cloth.y+cloth.h+rail*.49]){c.save();c.translate(x,y);c.shadowColor='rgba(0,0,0,.65)';c.shadowBlur=3;c.fillStyle='#e7ddc4';c.strokeStyle='#9c7b40';c.lineWidth=1;c.rotate(Math.PI/4);c.fillRect(-2.7,-2.7,5.4,5.4);c.strokeRect(-2.7,-2.7,5.4,5.4);c.restore();}}

    // Cushions: rubber shadow, saturated face, bevel highlight, stitched lower seam.
    const cushionTone=pool?'#0a5f58':'#075b3c',cushionMid=pool?'#14776d':'#0c7650',cushionHi=pool?'rgba(194,229,216,.52)':'rgba(199,234,211,.50)';c.lineJoin='round';c.lineCap='round';
  ...
  #poolBall(c,b,s,r,orientation=b.orientation){const stripe=b.poolGroup==='stripe',base=b.color||'#ddd';c.save();this.#ballShadow(c,s,r);const body=c.createRadialGradient(s.x-r*.38,s.y-r*.44,r*.04,s.x+r*.16,s.y+r*.18,r*1.18);body.addColorStop(0,'#fff');body.addColorStop(.08,'#fff');body.addColorStop(.17,stripe?'#fffef8':base);body.addColorStop(.60,stripe?'#f1efe5':base);body.addColorStop(.84,stripe?'#c7c6bd':base);body.addColorStop(1,stripe?'#6d7070':'#090e15');c.fillStyle=body;c.beginPath();c.arc(s.x,s.y,r,0,Math.PI*2);c.fill();
    ...
    c.strokeStyle='rgba(0,0,0,.50)';c.lineWidth=Math.max(1,r*.07);c.beginPath();c.arc(s.x,s.y,r,0,Math.PI*2);c.stroke();c.strokeStyle='rgba(232,225,204,.14)';c.lineWidth=Math.max(.7,r*.035);c.beginPath();c.arc(s.x,s.y,r*.91,-2.65,-.30);c.stroke();
  ...
  #snookerBall(c,b,s,r){c.save();this.#ballShadow(c,s,r);const edge=b.name==='Black'?'#000':b.kind==='red'?'#650817':b.color;const g=c.createRadialGradient(s.x-r*.39,s.y-r*.45,r*.03,s.x+r*.12,s.y+r*.16,r*1.16);g.addColorStop(0,'#fff');g.addColorStop(.075,'rgba(255,255,255,.98)');g.addColorStop(.16,b.color);g.addColorStop(.58,b.color);g.addColorStop(.82,edge);g.addColorStop(1,'#050705');c.fillStyle=g;c.beginPath();c.arc(s.x,s.y,r,0,Math.PI*2);c.fill();c.strokeStyle='rgba(0,0,0,.50)';c.lineWidth=Math.max(1,r*.07);c.stroke();c.strokeStyle='rgba(232,225,204,.13)';c.lineWidth=Math.max(.7,r*.034);c.beginPath();c.arc(s.x,s.y,r*.91,-2.62,-.32);c.stroke();this.#gloss(c,s,r);c.restore();}
  ...
  #guideCue(c,L,off=0){
    ...
    const g=c.createLinearGradient(tipX,tipY,buttX,buttY);g.addColorStop(0,'#f4e4b5');g.addColorStop(.34,'#d8b46b');g.addColorStop(.355,'#f8fbff');g.addColorStop(.385,'#90a7b8');g.addColorStop(.41,'#e7dcc2');g.addColorStop(.44,'#b69a5d');g.addColorStop(.51,'#4e7f69');g.addColorStop(.59,'#c9a354');g.addColorStop(.64,'#5b2d16');g.addColorStop(.74,'#9a4426');g.addColorStop(.88,'#241631');g.addColorStop(1,'#080b16');c.strokeStyle=g;c.lineWidth=7.3;c.beginPath();c.moveTo(tipX,tipY);c.lineTo(buttX,buttY);c.stroke();
    ...
    // Ferrule + chalked tip.
    const ferr=8;c.strokeStyle='#fff4d9';c.lineWidth=7.7;c.beginPath();c.moveTo(tipX,tipY);c.lineTo(tipX-sdCue.x*ferr,tipY-sdCue.y*ferr);c.stroke();c.strokeStyle='#4d9b78';c.lineWidth=7.9;c.beginPath();c.moveTo(tipX+sdCue.x*.7,tipY+sdCue.y*.7);c.lineTo(tipX+sdCue.x*3.2,tipY+sdCue.y*3.2);c.stroke();
    // Joint rings and wrapped butt bands.
    const ring=(t,col,w=1.6)=>{const x=tipX+(buttX-tipX)*t,y=tipY+(buttY-tipY)*t,nx=-sdCue.y,ny=sdCue.x;c.strokeStyle=col;c.lineWidth=w;c.beginPath();c.moveTo(x-nx*4.2,y-ny*4.2);c.lineTo(x+nx*4.2,y+ny*4.2);c.stroke();};ring(.36,'rgba(255,255,255,.95)',2);ring(.405,'rgba(23,33,42,.9)',1.8);ring(.59,'#cfad66',2);ring(.64,'rgba(255,255,255,.65)',1.3);for(let t=.77;t<.95;t+=.035)ring(t,t%0.07<.035?'rgba(181,81,52,.8)':'rgba(30,17,35,.78)',1.05);c.restore();
  }
  #proAim(c,L){
  ...
  #hints(c,L){if(this.ballInHand){c.save();c.fillStyle='rgba(12,37,28,.84)';rr(c,L.cx-42,L.cloth.y+8,84,22,11);c.fill();c.fillStyle='#ede8dc';c.font='900 8px Bahnschrift, Arial';c.textAlign='center';c.fillText('BALL IN HAND',L.cx,L.cloth.y+22);c.restore();}}

...
{
  const main=read('../src/main.js'),renderer=read('../src/render/Renderer2D.js'),html=read('../web/index.html'),css=read('../web/styles.css'),room=read('../gameRoom.js'),server=read('../server.js');
  assert.match(html,/id="mobileAimWheel"/);assert.match(main,/MOBILE_DEVICE/);assert.match(main,/mobileAimWheel/);assert.match(main,/cue\.angle-=da\*\.065/);
...
// Browser-native premium audio and clean-shot contracts.
{
  ...
  assert.match(html,/game\.js\?v=5\.8\.0-physics/);assert.match(html,/styles\.css\?v=5\.8\.0-physics/);
}
